...
public class IPLayerDemuxTest {
    ...
    void testChatProtocolDemux() throws Exception {
        ...
        byte[] chatData = new byte[]{0x01}; // TYPE_CHAT_SINGLE
        byte[] messageBytes = testMessage.getBytes("UTF-8");
        byte[] combined = new byte[chatData.length + messageBytes.length];
        System.arraycopy(chatData, 0, combined, 0, chatData.length);
        System.arraycopy(messageBytes, 0, combined, chatData.length, messageBytes.length);
    ...
    void testMultiplexing() throws Exception {
        ...
        byte[] chatPayload = new byte[]{0x01}; // TYPE_CHAT_SINGLE
        byte[] chatMsgBytes = chatMessage.getBytes("UTF-8");
        byte[] chatCombined = new byte[chatPayload.length + chatMsgBytes.length];
        System.arraycopy(chatPayload, 0, chatCombined, 0, chatPayload.length);
        System.arraycopy(chatMsgBytes, 0, chatCombined, chatPayload.length, chatMsgBytes.length);
        ...
        assertEquals(chatMessage, receivedChatMsg[0]);
