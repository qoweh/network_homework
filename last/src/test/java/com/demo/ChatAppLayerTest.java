...
public class ChatAppLayerTest {
    ...
    void testShortMessage() throws Exception {
        ...
        // 새로운 형식: "[우선순위] 메시지 (지연: Xms)" - 원본 메시지가 포함되어야 함
        assertTrue(receivedMessage[0].contains(testMessage), "수신 메시지에 원본이 포함되어야 함");
    }
    ...
    void testLongMessage() throws Exception {
        ...
        // 새로운 형식: "[우선순위] 메시지 (지연: Xms)" - 원본 메시지가 포함되어야 함
        assertTrue(receivedMessage[0].contains(testMessage), "재조립된 메시지에 원본이 포함되어야 함");
    }
    ...
    void testFragmentReordering() throws Exception {
        ...
        assertTrue(received, "메시지 수신 대기 시간 초과");
        // 새로운 형식: "[우선순위] 메시지 (지연: Xms)" - 원본 메시지가 포함되어야 함
        assertTrue(receivedMessage[0].contains(testMessage), "순서가 섞여도 재조립되어야 함");
    }
    ...
    void testDuplicateFragment() throws Exception {
        ...
        assertTrue(received);
        // 새로운 형식: "[우선순위] 메시지 (지연: Xms)" - 원본 메시지가 포함되어야 함
        assertTrue(receivedMessage[0].contains(testMessage), "중복 Fragment가 있어도 정상 재조립되어야 함");
    }
    ...
    void testKoreanMessage() throws Exception {
        ...
        boolean received = receiveLatch.await(3, TimeUnit.SECONDS);
        assertTrue(received);
        assertTrue(receivedMessage[0].contains(testMessage), "한글 메시지가 제대로 재조립되어야 함");
    }
}
